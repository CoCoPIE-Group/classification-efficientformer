...
def training_main(args_ai=None,**kwargs):
    parser = argparse.ArgumentParser(
    ...
    model = create_model(
        ...
        mobilenet_config_path=args.mobilenet_config_path,
    ...
    CL.init(args=args_ai, model=model, optimizer=optimizer, data_loader=data_loader_train)
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 5 == 4:
            ...
            if str(args.gpu)  == "0":
                xgen_record(args_ai, model.module, round(float(test_stats["acc1"]), 3), epoch)
